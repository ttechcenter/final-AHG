...
// Flat list of all header keys for iteration
const ALL_HEADERS = [
...
type HeaderKey = typeof ALL_HEADERS[number]['key'];
...
export function formatPlanAsText(items: PlanItem[], weekStart: string, name: string, dept: string): string {
  ...
  for (const day of DAYS) {
    ...
    for (const item of dayItems) {
      ...
      if (item.preparation) lines.push(`      Preparation: ${item.preparation}`);
      if (item.principle) lines.push(`      Principle: ${item.principle}`);
...
function AutoTextarea({
  ...
  return (
    <textarea
      ...
      onInput={resize}
      onPaste={onPaste}
      className="w-full min-w-[5rem] px-2 py-1.5 bg-transparent focus:bg-white focus:outline-none focus:ring-1 focus:ring-green-400 text-gray-800 text-xs resize-none overflow-hidden leading-relaxed"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[85vh] overflow-hidden flex flex-col">
        <div className="bg-green-800 px-6 py-4 flex items-center justify-between">
          ...
          <button onClick={onCancel} className="text-green-300 hover:text-white p-1">
            <X size={18} />
          </button>
        </div>
            ...
            <table className="min-w-full text-xs border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  {ALL_HEADERS.map((h) => (
                    <th key={h.key} className="px-3 py-2 text-left font-semibold text-gray-600 uppercase tracking-wide whitespace-nowrap">
                      {h.label}
                    </th>
                  ))}
                </tr>
...
function parseClipboardToRows(text: string): Partial<Record<HeaderKey, string>>[] {
  ...
  const keys = ALL_HEADERS.map((h) => h.key);
...
export default function WeeklyPlanTable({
  ...
  const handlePasteButton = async (day: string) => {
    ...
    try {
      ...
      if (rows.length === 0) {
        setPasteError('Could not parse clipboard data.');
        return;
      }
      setPasteDay(day);
      setPasteRows(rows);
        ...
        <table className="min-w-full text-xs border-collapse">
          ...
          <thead>
            <tr className="bg-green-800 text-white">
              <th className="px-3 py-2 text-left font-semibold border border-green-700 w-10 whitespace-nowrap print:border-black" rowSpan={2}>
                S/N
              </th>
              <th className="px-3 py-2 text-left font-semibold border border-green-700 w-24 whitespace-nowrap print:border-black" rowSpan={2}>
                Day
              </th>
              <th className="px-3 py-2 text-center font-semibold border border-green-700 print:border-black" rowSpan={2}>
                A/EPIE
              </th>
              <th className="px-3 py-2 text-center font-semibold border border-green-700 bg-blue-900 print:border-black" colSpan={3}>
                Preparation
              </th>
              <th className="px-3 py-2 text-center font-semibold border border-green-700 bg-orange-700 print:border-black" colSpan={3}>
                Implementation
              </th>
              <th className="px-3 py-2 text-center font-semibold border border-green-700 bg-purple-800 print:border-black" colSpan={3}>
                Evaluation
              </th>
              <th className="px-3 py-2 text-center font-semibold border border-green-700 w-12 whitespace-nowrap print:border-black" rowSpan={2}>
                Done
              </th>
            </tr>

            {/* Header Row 2 - Sub Headers */}
            <tr className="bg-green-700 text-green-100 text-[10px]">
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-blue-800 print:border-black">Preparation</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-blue-800 print:border-black">Principle</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-blue-800 print:border-black">Plan</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-orange-600 print:border-black">Perform</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-orange-600 print:border-black">Productivity</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-orange-600 print:border-black">Profit</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-purple-700 print:border-black">Pragmatism</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-purple-700 print:border-black">Persistence</th>
              <th className="px-2 py-1.5 text-center font-semibold border border-green-600 bg-purple-700 print:border-black">Profit</th>
            </tr>
          </thead>
            ...
            {DAYS.map((day) => {
              ...
              return dayItems.map((item, idx) => (
                <tr
                  ...
                  } hover:bg-green-50/60 transition-colors print:bg-white`}
                >
                  <td className="px-3 py-1.5 border border-gray-200 text-gray-500 align-top whitespace-nowrap print:border-black">
                    {item.s_no}
                  </td>

                  {idx === 0 ? (
                    <td
                      className="px-3 py-2 border border-gray-200 font-semibold text-green-800 bg-green-50 align-top print:border-black"
                  ...
                  {ALL_HEADERS.map((h) => (
                    <td key={h.key} className="border border-gray-200 p-0 align-top print:border-black">
                      {readOnly ? (
                        <div className="px-2 py-1.5 text-gray-700 whitespace-pre-wrap break-words leading-relaxed min-w-[5rem]">
                          {(item as any)[h.key] || '-'}
                        </div>
                      ) : (
                        <AutoTextarea
                          ...
                          onPaste={(e) => handleCellPaste(e, item.id, h.key)}
                        />
                      )}
                    </td>
                  ))}

                  <td className="border border-gray-200 px-3 py-1.5 text-center align-top print:border-black">
                    {readOnly ? (
                      item.is_completed ? (
